...
// Рекурсивная функция для вычисления интеграла
double kholin_k_multidimensional_integrals_rectangle_mpi::TestMPITaskSequential::integrate(
  ...
  if (curr_index_dim == dim_) {
    // Если достигли последнего измерения, возвращаем значение функции
    return f_(f_values_);
  }

  double sum = 0.0;
  for (size_t i = 0; i < n; ++i) {
    f_values_[curr_index_dim] =
        l_limits[curr_index_dim] + (i + 0.5) * h[curr_index_dim];  // Центр текущего прямоугольника
    sum += integrate(f_, l_limits, u_limits, h, f_values_, curr_index_dim + 1, dim_,
                     n);  // Рекурсивный вызов для следующего измерения
  }
  return sum * h[curr_index_dim];  // Умножаем на шаг по текущему измерению
}
...
double kholin_k_multidimensional_integrals_rectangle_mpi::TestMPITaskSequential::integrate_with_rectangle_method(
  ...
  return integrate(f_, l_limits, u_limits, h, f_values_, 0, dim_,
                   n);  // Начинаем интеграцию с первого измерения
}
...
double kholin_k_multidimensional_integrals_rectangle_mpi::TestMPITaskSequential::run_multistep_scheme_method_rectangle(
  ...
  int n = 1;  // Начальное количество шагов
  double I_n = integrate_with_rectangle_method(f_, f_values_, l_limits, u_limits, dim_, n);
  double I_2n;
  double delta = 0;
  do {
    n *= 2;  // Увеличиваем количество шагов
    I_2n = integrate_with_rectangle_method(f_, f_values_, l_limits, u_limits, dim_, n);
    delta = std::abs(I_2n - I_n);  // Оценка погрешности
    I_n = I_2n;  // Обновляем значение интеграла для следующей итерации

    // Выводим результаты для текущего шага
    std::cout << "n: " << n << ", I: " << I_2n << ", delta : " << delta << std::endl;

  } while ((1.0 / 3) *delta >= epsilon_);  // Проверяем условие остановки

  return I_2n;  // Возвращаем последнее значение интеграла
}
...
double kholin_k_multidimensional_integrals_rectangle_mpi::TestMPITaskParallel::run_multistep_scheme_method_rectangle(
  ...
  double local_result;
  double delta = 0;
  // Ограничиваем количество процессов до общего количества шагов
  ...
  do {
    n *= 2;  // Увеличиваем количество шагов
    // Вычисляем интеграл для локальных пределов
